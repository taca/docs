...
![The Rich Text Editor TinyMCE](/ja/article-management/images/en/the-tinymce-rich-text-editor.png?classes=shadow)
...
![Adding an image to a text](/ja/article-management/images/en/add-an-image-to-a-text.png?classes=shadow)
...
**Image alignment:** Here you can set the alignment of the image. If it is inserted ![above](/ja/icons/above.svg?classes=icon) **above**, ![under](/de/icons/below.svg?classes=icon) **below**, ![left-justified](/de/icons/left.svg?classes=icon) **left-aligned** or ![right-justified](/de/icons/right.svg?classes=icon) **right-aligned**. When **left-** or **right-aligned**, the text **flows around** the image (as symbolized by the icon).
...
![JavaScript wizard for listings](/ja/article-management/images/en/javascript-assistant-for-lists.png?classes=shadow)

With a click on the icon next ![Import list data from a CSV file](/ja/icons/tablewizard.svg?classes=icon) to the field name "List entries" you open the CSV import wizard, with which you can import list data from a CSV file. The CSV file must have been transferred to the upload directory before.
...
- ![Reduce the size of the input fields](/ja/icons/demagnify.svg?classes=icon) **Reduce the size of the input fields**
- ![Enlarge the input fields](/ja/icons/magnify.svg?classes=icon) **Enlarge the input fields**
- ![Duplicate the column/row](/ja/icons/copy.svg?classes=icon) **Duplicate the column/row**
- ![Move the column one position to the left](/ja/icons/movel.svg?classes=icon)**Move the column one position to the left**
- ![Move the column one position to the right](/ja/icons/mover.svg?classes=icon)**Move the column one position to the right**
- ![Delete the column/row](/ja/icons/delete.svg?classes=icon) **Delete the column/row**
- ![Move the element by dragging and dropping it](/ja/icons/drag.svg?classes=icon)**Move the row by dragging and dropping it**

![JavaScript wizard for tables](/ja/article-management/images/en/javascript-assistant-for-tables.png?classes=shadow)

With a click on the icon next ![Import list data from a CSV file](/ja/icons/tablewizard.svg?classes=icon) to the field label "Table entries" you open the CSV import wizard, with which you can import table data from a CSV file. The CSV file has to be transferred to the upload directory first.
...
```md
![Alt text](/ja/pfad/zum/bild.jpg "Optionaler Titel")
```
