...
ダウンロードに成功すると`contao-manager.phar`というファイルができますので、このファイルをウェブサーバーの`web` ディレクトリに転送してください。
...
![Contao Managerの開始ページ](/ja/installation/images/en/welcomepage-contao-manager.png?classes=shadow)
...
![サーバー構成](/ja/installation/images/en/server-configuration.png?classes=shadow)
