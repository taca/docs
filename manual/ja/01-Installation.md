...
テンプレートは構成済みのウェブサイトのサンプルで、サイト構造、Contaoのコアモジュールとコンテント要素の書式を整形するいくつかのスタイルシートを含んでいます。初期状態では"Music Academy"というテンプレートがあります。これをインポートするにはドロップダウンメニューから`example_website.sql`を選択して、「テンプレートをインポート」のボタンをクリックしてください。
