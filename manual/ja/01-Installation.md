...
ContaoにはPHPとMySQLをサポートしている、ApacheやIISのようなウェブサーバーが必要です。PHPの最小のバージョンは5.3.2、MySQLの最小のバージョンは5.0.3ですが、よりよい性能を得るためにMySQL 5の使用を推奨します。また、PHPの拡張の"GDlib"(画像の大きさの変更)と"SOAP"(機能拡張リポジトリ)も必要で、さらに"mbstring"(マルチバイト文字の処理)と"mcrypt"(データの暗号化)も追加で必要な場合があります。Contaoはすべての主要なブラウザー、Firefox(バージョン2以降)やInternet Explore(バージョン7以降)で正しく動作するテストをしています。
